...
class fixation_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for root, dirs, files in os.walk(self.img_ftrs_dir):
            for file in files:
                if file == fixation['img_name'].replace('jpg', 'pth'):
                    image_ftrs = torch.load(join(root, file)).unsqueeze(0)
                    break

        
        return {'tgt_y': fixation['tgt_seq_y'].float(), 'tgt_x': fixation['tgt_seq_x'].float(), 
                'tgt_t': fixation['tgt_seq_t'].float(),
                'src_img': image_ftrs, 'class': fixation['class']} 
...
class COCOSearch18Collator(object):
    ...
    def __call__(self, batch):
        ...
        for t in batch:
            ...
            if t['class'] == 'freeview':
                batch_classes.append([1, 0])
            else:
                batch_classes.append([0, 1])
        ...
        batch_tgt = torch.cat([batch_tgt_y, batch_tgt_x, batch_tgt_t], dim = -1).permute(1, 0, 2)
        ...
        return batch_imgs, batch_tgt, batch_tgt_padding_mask, batch_firstfix, batch_classes
