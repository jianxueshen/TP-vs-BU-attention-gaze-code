...
class gazeformer(nn.Module):
    def __init__(self, transformer, spatial_dim, dropout=0.4, max_len = 7, patch_size  = 16, device = "cuda:0"):
        ...
        self.queryfix_embed = nn.Embedding(max_len, 3).to(device)
        ...
        self.classfier = nn.Linear(self.hidden_dim, 2).to(device)
    ...
    def forward(self, src: Tensor, tgt: Tensor):
        ...
        outs = self.transformer(src=src, tgt=tgt, tgt_mask= None, tgt_key_padding_mask = None, 
                querypos_embed = self.querypos_embed.weight.unsqueeze(1), 
                patchpos_embed = self.patchpos_embed)

        outs = self.dropout(outs)
        # Aggregate the sequence dimension (dim=0) using mean
        outs = torch.mean(outs, dim=0)
        logits = self.classfier(outs)
