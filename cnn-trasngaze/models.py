...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, img_hidden_dim = 2048, nhead=8, num_encoder_layers=6,
        ...
        self.encoder =  TransformerEncoderWrapper(encoder, input_proj, img_transform, encoder_dropout, device).to(device)
        ...
        inputgt_proj = nn.Linear(3, d_model).to(device)
        dropout = nn.Dropout(decoder_dropout)
        self.decoder = TransformerDecoderWrapper(d_model, activation, decoder, inputgt_proj,
                                                 dropout, device)
...
class TransformerEncoderWrapper(nn.Module):
    ...
    def __init__(self, encoder, input_proj, img_transform, dropout, device):
        super().__init__()
        self.device = device
    ...
    def forward(self, src,
        ...
        #src_flat = src.view(batch_size * seq_len, -1)
        #src_proj_flat = self.input_proj(src_flat)
        
        src_proj = self.input_proj(src).permute(1,0,2)#input projection from 2048 -> d
        #src_proj = src_proj_flat.view(batch_size, seq_len, -1).permute(1, 0, 2)  # [seq_len, batch_size, d_model]
...
class TransformerDecoderWrapper(nn.Module):
    def __init__(self, d_model, activation, decoder, input_proj, dropout, device):
        super().__init__()
        self.device = device
    ...
    def forward(self, tgt, memory,
                ...
                patchpos_embed: Optional[Tensor] = None):
        #decoder
        tgt_proj = self.input_proj(tgt).permute(1,0,2)#input projection from 3 -> d
